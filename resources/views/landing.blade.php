        ...
        <div class="relative min-h-screen overflow-hidden">
            ...
            <header class="flex items-center justify-between rounded-full border border-slate-800/80 bg-slate-900/70 px-5 py-3 shadow-lg shadow-black/20 backdrop-blur">
                ...
                <nav class="flex items-center gap-3 text-sm text-slate-300">
                    @if (Route::has('login'))
                        @auth
                            <a href="{{ url('/dashboard') }}" class="rounded-full border border-cyan-500/40 bg-cyan-500/10 px-4 py-2 font-medium text-cyan-300 transition hover:bg-cyan-500/20">
                                Dashboard
                            </a>
                        @else
                            <a href="{{ route('login') }}" class="rounded-full px-4 py-2 transition hover:bg-slate-800 hover:text-white">
                                Log in
                            </a>
            ...
            <main class="flex-1 py-12 lg:py-20">
                <section class="grid items-center gap-10 lg:grid-cols-[1.1fr_0.9fr]">
                    <div>
                        <div class="mb-6 inline-flex items-center rounded-full border border-cyan-500/20 bg-cyan-500/10 px-3 py-1 text-sm font-medium text-cyan-300">
                        ...
                        <div class="mt-8 flex flex-wrap gap-4">
                            ...
                            @endif
                            @if (Route::has('register'))
                                <a href="{{ route('register') }}" class="rounded-full border border-slate-700 bg-slate-900 px-6 py-3 font-semibold text-white transition hover:bg-slate-800">
                                    Request access
                                </a>
                            @endif
                        </div>
                        ...
                        <div class="mt-10 grid gap-4 sm:grid-cols-3">
                            <div class="rounded-2xl border border-slate-800 bg-slate-900/70 p-4">
                                ...
                                <p class="mt-1 text-sm text-slate-400">Live stock visibility</p>
                            </div>
                            <div class="rounded-2xl border border-slate-800 bg-slate-900/70 p-4">
                                <p class="text-2xl font-semibold text-white">99.2%</p>
                                <p class="mt-1 text-sm text-slate-400">Order accuracy</p>
                            </div>
                            <div class="rounded-2xl border border-slate-800 bg-slate-900/70 p-4">
                                <p class="text-2xl font-semibold text-white">3x</p>
                                <p class="mt-1 text-sm text-slate-400">Faster approvals</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="rounded-3xl border border-slate-800 bg-gradient-to-br from-slate-900 via-slate-900 to-slate-800 p-6 shadow-2xl shadow-black/30">
                        <div class="flex items-center justify-between rounded-2xl border border-slate-700 bg-slate-950/70 px-4 py-3">
                            <div>
                ...
                <section class="mt-20 grid gap-6 md:grid-cols-3">
                    <div class="rounded-3xl border border-slate-800 bg-slate-900/70 p-6">
                        ...
                        <p class="mt-3 text-sm leading-7 text-slate-400">Create purchase requests, review supplier quotes, and keep approvals moving without bottlenecks.</p>
                    </div>
                    <div class="rounded-3xl border border-slate-800 bg-slate-900/70 p-6">
                        <h2 class="text-xl font-semibold text-white">Inventory visibility</h2>
                        <p class="mt-3 text-sm leading-7 text-slate-400">Monitor stock across locations, identify low units, and manage movement with accurate updates.</p>
                    </div>
                    <div class="rounded-3xl border border-slate-800 bg-slate-900/70 p-6">
                        <h2 class="text-xl font-semibold text-white">Operations reporting</h2>
                        <p class="mt-3 text-sm leading-7 text-slate-400">Turn warehouse activity into concise dashboards for planning, audits, and executive review.</p>
                    </div>
                </section>
            </main>
            </div>
